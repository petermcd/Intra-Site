...
class Monitoring:
    ...
    def create_device(self, device_details):
        ...
        templates = self._format_templates(device_details)
    ...
    def update_device(self, device_details):
        ...
        if not device:
            ...
            return
        required_templates, templates_to_clear = self._format_templates(device, device_details)
        groups = self._format_groups(device_details)
    ...
    @staticmethod
    def _format_templates(device, new_device_details):
        ...
        for template in device['parentTemplates']:
            if template['templateid'] not in required_template_ids:
                templates_to_clear.append({'templateid': template['templateid']})
